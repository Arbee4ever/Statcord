...
public class GuildVoiceEvents extends ListenerAdapter {
    ...
    public void onGuildVoiceUpdate(GuildVoiceUpdateEvent event) {
        ...
        if (event.getChannelJoined() != null && event.getChannelLeft() == null) {
            ...
            StatcordBot.logger.info("Someone joined VC! bot=" + user.isBot() + " Name: " + user.getEffectiveName());
        ...
        if (event.getChannelJoined() == null && event.getChannelLeft() != null) {
            User user = event.getMember().getUser();
            Date lastjoin = (Date) Data.getMemberValue(event.getMember().getUser(), event.getGuild(), "voicestart");
            StatcordBot.logger.info("Someone left VC! bot=" + user.isBot() + " Name: " + user.getEffectiveName() + " Joined at: " + lastjoin + " Left at: " + new Timestamp(System.currentTimeMillis()));
